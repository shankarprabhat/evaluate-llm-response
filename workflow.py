...
async def main():
    ...
    workflow = RAGWorkflow(llm_choice="Qwen3")
    
    # Ingest documents
    await workflow.ingest_documents("source")

    df_question = pd.read_excel("Q&A Training Data Use Case 3.xlsx",sheet_name="21CFR Part 50")
    question_list = df_question['Question'].tolist()
    ground_truth_list = df_question['Answer'].tolist()

    question_list = question_list[:2]  # Limit to first 10 questions for testing
    ground_truth_list = ground_truth_list[:2]  # Limit to first 10 answers for testing
...
model_name = ["qwen.6b","deepseek-r1"]
data_sheet = ["21CFR Part 50","21CFR Part 56","21CFR Part 58","NITL01","AZD9291","ICH GCP E6 R3"]
